...
void Autons::solo()
{
  chassisReference->odometry->setPosition(-47, -15.5, 180);
  ...
  chassisReference->driveToPoint(Pose<double>(-25, -21.5, 0), {.driveSettleError = 1, .driveSettleTime = 100, .driveTimeout = 1300}, {}, {});

  blocker.off();

  // Go up to low goal
  willyNilly.delayToggle(100);
  chassisReference->turnTo(Pose<double>(-12, -10, -360), {}, {});
  intake.stopFullIntake();
  IntakeFront.spin(vex::directionType::fwd, 12, volt);
  chassisReference->driveToPoint(Pose<double>(-12, -10, 0), {.driveTimeout = 1000}, {}, {});
  razer.on();
