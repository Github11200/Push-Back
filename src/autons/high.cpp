...
void Autons::high()
{
  ...
  // Ram into loader
  intake.spinFullIntake(vex::directionType::fwd);
  chassisReference->driveToPoint(Pose<double>(-70, 46.6, 180), {.driveMaxVoltage = 5, .driveTimeout = 1300}, {}, {});

  // Line up to long goal
  chassisReference->turnTo(Pose<double>(-25, 49.3, -360), {}, {});
  ...
  sloper.on();
  wait(1200, msec);
  chassisReference->odometry->setPosition(-28.5, 47, chassisReference->getAbsoluteHeading().angle + 90);
  sloper.off();

  // Back away from goal
  chassisReference->driveToPoint(Pose<double>(-38, 47, 0), {.driveTimeout = 1000}, {}, {});

  // Reposition to align wing
  chassisReference->turnTo(Pose<double>(-37, 33.5, -360), {}, {});
