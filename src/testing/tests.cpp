...
// ====================
// INTERPOLATED LUT TESTS (Monotonic Cubic Splines)
// ====================

TEST(testInterpLUTLinearData)
{
  // Test with linear data points
  ...
  InterpolatedLUT lut(points);

  // Test exact points
  ASSERT_EQUAL(lut.interpolate(0), 0);
  ASSERT_EQUAL(lut.interpolate(1), 1);
  ASSERT_EQUAL(lut.interpolate(2), 2);
  ASSERT_EQUAL(lut.interpolate(3), 3);

  // Test interpolation between points
  double mid = lut.interpolate(0.5);
  ASSERT_TRUE(mid > 0 && mid < 1); // Should be between 0 and 1

  return Testing::Result::PASS;
}

TEST(testInterpLUTQuadraticData)
{
  // Test with quadratic data (x^2)
  vector<Vector2D<double>> points = {
      {0, 0},
      {1, 1},
      {2, 4},
  ...
  ASSERT_EQUAL(lut.interpolate(0), 0);
  ASSERT_EQUAL(lut.interpolate(1), 1);
  ASSERT_EQUAL(lut.interpolate(2), 4);
  ASSERT_EQUAL(lut.interpolate(3), 9);

  // Interpolation should give reasonable values
  double mid = lut.interpolate(1.5);
  ASSERT_TRUE(mid > 1 && mid < 4); // Should be between adjacent points

  return Testing::Result::PASS;
}

TEST(testInterpLUTBoundaryBelowMin)
{
  // Test point below minimum x value
  vector<Vector2D<double>> points = {
      {1, 10},
      {2, 20},
      {3, 30}};
  ...
  // Points below minimum should return first y value
  ASSERT_EQUAL(lut.interpolate(-5), 10);
  ASSERT_EQUAL(lut.interpolate(0), 10);
  ASSERT_EQUAL(lut.interpolate(0.5), 10);

  return Testing::Result::PASS;
}

TEST(testInterpLUTBoundaryAboveMax)
{
  // Test point above maximum x value
  vector<Vector2D<double>> points = {
      {0, 5},
      {1, 15},
      {2, 25}};
  ...
  // Points above maximum should return last y value
  ASSERT_EQUAL(lut.interpolate(2), 25);
  ASSERT_EQUAL(lut.interpolate(3), 25);
  ASSERT_EQUAL(lut.interpolate(10), 25);

  return Testing::Result::PASS;
}

TEST(testInterpLUTTwoPoints)
{
  // Edge case: only two points (simple linear interpolation)
  vector<Vector2D<double>> points = {
      {0, 0},
      {10, 10}};
  ...
  ASSERT_EQUAL(lut.interpolate(0), 0);
  ASSERT_EQUAL(lut.interpolate(10), 10);

  // Linear interpolation at midpoint
  double mid = lut.interpolate(5);
  ASSERT_EQUAL(mid, 5);

  return Testing::Result::PASS;
}

TEST(testInterpLUTNegativeSlope)
{
  // Test with decreasing y values (negative slope)
  vector<Vector2D<double>> points = {
      {0, 100},
      {1, 75},
      {2, 50},
  ...
  ASSERT_EQUAL(lut.interpolate(0), 100);
  ASSERT_EQUAL(lut.interpolate(3), 25);

  double mid = lut.interpolate(1.5);
  ASSERT_TRUE(mid < 75 && mid > 25); // Should be between boundary points

  return Testing::Result::PASS;
}

TEST(testInterpLUTHorizontalSegment)
{
  // Test with horizontal segment (zero slope)
  vector<Vector2D<double>> points = {
      {0, 10},
      {1, 10},
      {2, 10},
  ...
  // Values in flat region should be approximately constant
  ASSERT_EQUAL(lut.interpolate(0), 10);
  ASSERT_EQUAL(lut.interpolate(1), 10);
  ASSERT_EQUAL(lut.interpolate(2), 10);

  double mid = lut.interpolate(0.5);
  ASSERT_TRUE(mid >= 10 && mid <= 10.1); // Nearly 10

  return Testing::Result::PASS;
}

TEST(testInterpLUTSteepVsGradual)
{
  // Test monotonicity: alternating steep and gradual slopes
  vector<Vector2D<double>> points = {
      {0, 0},
      {1, 10}, // steep
      {2, 11}, // gradual
  ...
  ASSERT_EQUAL(lut.interpolate(0), 0);
  ASSERT_EQUAL(lut.interpolate(1), 10);
  ASSERT_EQUAL(lut.interpolate(2), 11);
  ASSERT_EQUAL(lut.interpolate(3), 21);

  return Testing::Result::PASS;
}

TEST(testInterpLUTMonotonicity)
{
  ...
  // Sample points and verify monotonically increasing
  double prev = lut.interpolate(0);
  for (double x = 0.1; x <= 4; x += 0.1)
  {
    double curr = lut.interpolate(x);
    ASSERT_TRUE(curr >= prev - 1e-9); // Allow small numerical error
    prev = curr;
  }
...
TEST(testInterpLUTNegativeCoordinates)
{
...
TEST(testInterpLUTDecreasingMonotonicity)
{
...
TEST(testInterpLUTClosePoints)
{
  ...
  ASSERT_EQUAL(lut.interpolate(0), 0);
  ASSERT_EQUAL(lut.interpolate(0.001), 0.001);
  ASSERT_EQUAL(lut.interpolate(0.002), 0.002);
  ASSERT_EQUAL(lut.interpolate(1), 1);

  return Testing::Result::PASS;
}

TEST(testInterpLUTLargeRange)
{
  // Test with large value ranges
  vector<Vector2D<double>> points = {
      {0, 0},
      {100, 1000},
      {200, 2500},
  ...
  ASSERT_EQUAL(lut.interpolate(0), 0);
  ASSERT_EQUAL(lut.interpolate(100), 1000);
  ASSERT_EQUAL(lut.interpolate(200), 2500);
  ASSERT_EQUAL(lut.interpolate(300), 5000);

  // Interpolation should be bounded
  double mid = lut.interpolate(150);
  ASSERT_TRUE(mid > 1000 && mid < 2500);

  return Testing::Result::PASS;
}

TEST(testInterpLUTCubicShape)
{
  // Test with cubic-like data
  vector<Vector2D<double>> points = {
      {-2, -8},
      {-1, -1},
      {0, 0},
  ...
  // Test exact points
  ASSERT_EQUAL(lut.interpolate(-2), -8);
  ASSERT_EQUAL(lut.interpolate(-1), -1);
  ASSERT_EQUAL(lut.interpolate(0), 0);
  ASSERT_EQUAL(lut.interpolate(1), 1);
  ASSERT_EQUAL(lut.interpolate(2), 8);
  ...
  // Test interpolation
  double mid1 = lut.interpolate(0.5);
  double mid2 = lut.interpolate(-0.5);

  ASSERT_TRUE(mid1 > 0 && mid1 < 1);
  ASSERT_TRUE(mid2 < 0 && mid2 > -1);

  return Testing::Result::PASS;
}

TEST(testInterpLUTMultipleSlopesZero)
{
  // Test with multiple zero slopes (flat regions)
  vector<Vector2D<double>> points = {
      {0, 5},
      {1, 5},
      {2, 5},
  ...
  ASSERT_EQUAL(lut.interpolate(0), 5);
  ASSERT_EQUAL(lut.interpolate(1), 5);
  ASSERT_EQUAL(lut.interpolate(2), 5);
  ASSERT_EQUAL(lut.interpolate(3), 10);
  ASSERT_EQUAL(lut.interpolate(4), 10);
  ASSERT_EQUAL(lut.interpolate(5), 10);

  return Testing::Result::PASS;
}

TEST(testInterpLUTContinuity)
{
  // Test that interpolation is continuous (no jumps)
  vector<Vector2D<double>> points = {
      {0, 0},
      {1, 2},
      {2, 3},
  ...
  // Sample densely and check no big jumps
  double maxJump = 0;
  double prev = lut.interpolate(0);
  for (double x = 0.01; x <= 3; x += 0.01)
  {
    double curr = lut.interpolate(x);
    double jump = abs(curr - prev);
    if (jump > maxJump)
      maxJump = jump;
    prev = curr;
  }

  ASSERT_TRUE(maxJump < 0.5); // Reasonable continuity

  return Testing::Result::PASS;
}

TEST(testInterpLUTVelocitySmoothness)
{
  // Test smoothness by checking approximate derivatives
  vector<Vector2D<double>> points = {
      {0, 0},
      {1, 1},
      {2, 4},
  ...
  // Sample derivative approximation
  double h = 0.001;
  double x = 1.5;
  double deriv1 = (lut.interpolate(x + h) - lut.interpolate(x - h)) / (2 * h);
  double deriv2 = (lut.interpolate(x + 2 * h) - lut.interpolate(x + h)) / h;

  // Second derivative should be small (smooth curve)
  double secondDeriv = (deriv2 - deriv1) / h;
  ASSERT_TRUE(abs(secondDeriv) < 10); // Should be relatively smooth
