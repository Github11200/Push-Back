...
struct TrackerPositions {
  ...
  TrackerPositions(double forward, double sideways)
...
enum Wall { FRONT = 0, RIGHT = 1, LEFT = 2, REAR = 3 };
...
public:
  ...
  /**
   * @brief Stops the position tracking thread, this function should typically
   * be run when drive control starts
   */
   ...
   * Uses the Pilons arc method outline here:
   * https://wiki.purduesigbots.com/software/odometry Also uses Monte Carlo
   * Localization:
  ...
  /**
   * @brief Resets the robot position, usually done during initialization at the
   * start of the match
   *
   * @param xPosition The new x position
   * @param yPosition The new y position
   * @param theta The new orientation (e.g. what orientation the robot would be
   * starting at for the start of the match)
   */
  ...
  void getWheelDiameters(int forwardOrSidewaysTracker,
